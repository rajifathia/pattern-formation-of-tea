...
# ╔═╡ 36c59fe5-680c-4918-abc3-93c6d9428b67
begin
    using PlutoUI
using WGLMakie
...
# ╔═╡ b73fa97d-2d11-40a1-9886-b387190726ff
# ╠═╡ disabled = true
#=╠═╡

  ╠═╡ =#

# ╔═╡ bc31aed0-a469-11f1-a350-dff014ab246e
begin
    # Cup dimensions
    R = 1.0
    H = 2.0
    ...
    # Reference temperature difference for visible convection
    ΔT_critical = 20.0
end

# ╔═╡ a6bf5120-cfc3-4d57-a57b-30905d764f3a
begin
    md"""
    # Interactive 3D Tea Convection

    Adjust the temperatures at the bottom and top of the tea.

    When the temperature difference is small, heat is transferred
    mainly by conduction.

    When the temperature difference becomes sufficiently large,
    buoyancy can drive convection.
    """
end

# ╔═╡ d1d1ffbc-d1a6-44c7-b615-389c5ec06459


# ╔═╡ 3924e817-3a42-4947-a26a-9490b5f4398c
@bind T_bottom PlutoUI.Slider(
    Tmin:Tmax,
    default = 60,
    show_value = true
)

# ╔═╡ b9219256-7555-4075-b944-ce22ecdb6d64
@bind T_top PlutoUI.Slider(
    Tmin:Tmax,
    default = 40,
    show_value = true
)

# ╔═╡ 21047b29-d657-42fd-bc0c-bdac52def744
md"""
## Temperature controls

**Bottom of the tea:** $(T_bottom) °C

**Top of the tea:** $(T_top) °C

**Current Temperature difference:** $(round(T_bottom - T_top, digits=1)) °C

---

### What this means

- **Small temperature difference:** mostly conduction.
- **Large temperature difference:** stronger buoyancy and convection.
"""

# ╔═╡ 36360669-4791-4253-9bd9-4681eb5fede2
begin
    ΔT = T_bottom - T_top

    # Use the absolute temperature difference for the
    # strength of the visual circulation.
    convection_strength = clamp(
        abs(ΔT) / ΔT_critical,
        0.0,
        1.0
    )
    ...
    if ΔT > ΔT_critical
        regime = "Convection is strong"
    elseif ΔT > 5
        regime = "Convection is developing"
    else
        regime = "Mostly conduction"
    end
end

# ╔═╡ 7cd85a58-71b4-4a79-85ff-b11a861d9216


# ╔═╡ ede5355f-bae7-4a35-bf9e-d4ecf41b5c63
begin
    fig = Figure(
        size = (950, 750),
    ...
    ax = Axis3(
        ...
        ylabel = "y",
        zlabel = "height",
        title = "3D temperature structure of the tea",
        aspect = (1, 1, 1)
    )
    ...
    # Temperature varies from bottom to top.
    temperature = [
        T_bottom + (T_top - T_bottom) * (zi / H)
        for θi in θ, zi in z
    ]
    ...
    for k in 1:nz
        ...
        for j in 1:nθ
            ...
            for i in 1:nr
                ...
                # ------------------------------------------------
                # Artificial visual circulation
                #
                # This is NOT a new PDE simulation.
                # It simply makes the buoyancy/convection idea
                ...
                x = radius * cos(angle) + circulation * sin(angle)

                y = radius * sin(angle) + circulation * cos(angle)
                ...
                # Add a small temperature variation when
                # convection becomes strong.
                T_visual =
                    local_temperature +
                    convection_strength *
                    5.0 *
    ...
    # --------------------------------------------------------
    # TOP AND BOTTOM TEMPERATURE LABELS
    # --------------------------------------------------------
    ...
    text!(
        ...
        0,
        H + 0.15,
        text = "Top: $(round(T_top, digits=1)) °C",
        align = (:center, :center),
        fontsize = 18,
        color = :blue
    )
    ...
    text!(
        ...
        0,
        -0.15,
        text = "Bottom: $(round(T_bottom, digits=1)) °C",
        align = (:center, :center),
        fontsize = 18,
        color = :red
    )
...
# ╔═╡ b9ab3c24-1783-4827-a0f4-592f16cecde9
md"""
## What is happening?
...
\[
\Delta T = $(round(ΔT, digits=1))^\circ C
\]
...
The bottom of the tea is warmer than the top.

Warm tea tends to rise, while cooler tea tends to sink.

If the temperature difference is small, this movement is weak and
heat is transferred mainly through the tea without strong circulation.

As the temperature difference increases, the buoyancy becomes stronger
and the tea can begin to circulate.

### The important idea

\[
\boxed{
\text{larger temperature difference}
\rightarrow
\text{stronger buoyancy}
\rightarrow
\text{stronger circulation}
}
\]

This is the physical mechanism behind the convection studied in the
main Oceananigans simulation.
"""

# ╔═╡ 97c9e051-15b5-416c-855d-8561d01a6750


# ╔═╡ f7b70f9e-2704-431e-84b5-579f79ab7065
@bind dye_on PlutoUI.CheckBox(default = false)

# ╔═╡ 0d96e7e3-a8a5-48da-8583-2de096f1f46e
md"""
## Dye experiment

Turn on the dye to place a small drop inside the tea.

**Dye experiment:** $(dye_on ? "ON" : "OFF")

The dye is passive: it does not make the tea move.
It simply helps us see how the moving tea carries material with it.
"""

# ╔═╡ f0e10a1c-0bc9-4109-88fe-a42079da085a
@bind dye_time PlutoUI.Slider(
    0:0.1:20,
    default = 0,
    show_value = true
)

# ╔═╡ f83268d7-d48e-4dea-9be4-3064c267d407
md"""
### Dye evolution

**Time:** $(round(dye_time, digits = 1)) s

Move the slider to see how the dye changes position as the tea circulates.
"""

# ╔═╡ 16a997d9-2aa2-40b4-8f6a-ab557ea56aa6
begin
    ...
    ΔT_current = T_bottom - T_top

    # --------------------------------------------------------
    # Dimensionless convection strength
    #
    # This controls only the educational visualization.
    # It is not replacing the Oceananigans simulation.
    # --------------------------------------------------------

    convection_strength2 = clamp(
        max(ΔT_current, 0.0) / 20.0,
        0.0,
        1.0
    )

    # --------------------------------------------------------
    # Initial dye position
    # --------------------------------------------------------
    ...
    # Dye motion
    #
    # The dye follows a simple circulating path.
    # The amount of movement depends on the temperature
    # difference.
    # --------------------------------------------------------

    phase = 0.8 * convection_strength * dye_time
...
# ╔═╡ d6851eeb-ffaa-48aa-b1cd-d706a0ce532d
if dye_on
...
manifest_format = "2.0"
project_hash = "235cd2377cbacd1361f2eee05e28cce711ed44a5"
